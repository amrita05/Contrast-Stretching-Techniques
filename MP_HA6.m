...
for R=1:rows
    for C=1:columns
        pixel_img=im(R,C);
        if pixel_img<=255 && pixel_img > b
            pixel_img=Tb + (((255-Tb)/(255-b))*(pixel_img-b));
        end
        if pixel_img<=b && pixel_img > a
            pixel_img=Ta +(((Tb-Ta)/(b-a))*(pixel_img-a));
        end
        if pixel_img<=a
            pixel_img=(Ta/a)*pixel_img;
        end
        otra(R,C)=pixel_img;
    end
end
...
numofpixel_imgs=size(im,1)*size(im,2);
...
%freq counts the occurrence of each pixel_img value.
...
for i=1:size(im,1)
    ...
    for j=1:size(im,2)
        ...
        probf(value+1)=freq(value+1)/numofpixel_imgs;
